...
// 리랜더링
const Block = ({num}) => {
    // 아니면 {num} 이렇게 받아와서 객체 안에 값을 가져온다.
    // ㅜ .을 사용해서 받을 수 있다.
    // console.log(num.num);
    let count = 0;
    ...
    // useState함수이 매개변수가 초기 세팅값
    // setnum 함수를 사용해서 값을 수정할 땐 setnum 함수의 매개변수로 전달
    // 함수사용
    // 일반변수는 다시 그려주면 초기 값으로 변한다.
    // 이유는 리랜더링 하기 때문인데 하지만 useState 값은
  ...
  return (
    ...
    {/* 변수를 바꾼다고 태그에 변수값 넣은게 바뀌지 않았다.
        ...
        변수를 전부 보고 다시 그린다고하면 처음에는 dom그리는 비용이 생각보더
        비싼데 이걸 많이 그려주다보면 컴퓨터가 미치기 때문에 줄여주는 것.
